...
class TensorMG(TensorBase):
    '''
        MERF, MG = Matrix + Grid
    '''
    def __init__(self, aabb, gridSize, device, **kargs):
        super(TensorMG, self).__init__(aabb, gridSize, device, **kargs)
    ...
    def get_optparam_groups(self, lr_init_spatialxyz = 0.02, lr_init_network = 0.001):
        grad_vars = [{'params': self.density_grid, 'lr': lr_init_spatialxyz}, {'params': self.density_plane, 'lr': lr_init_spatialxyz},
                     {'params': self.app_grid, 'lr': lr_init_spatialxyz}, {'params': self.app_plane, 'lr': lr_init_spatialxyz},
                         {'params': self.basis_mat.parameters(), 'lr':lr_init_network}]
        if isinstance(self.renderModule, torch.nn.Module):
            grad_vars += [{'params':self.renderModule.parameters(), 'lr':lr_init_network}]
        return grad_vars
    ...
    def vector_comp_diffs(self):
        return self.vectorDiffs(self.density_grid) + self.vectorDiffs(self.app_grid)
    
    def density_L1(self):
        total = 0
        for idx in range(len(self.density_plane)):
            total = total + torch.mean(torch.abs(self.density_plane[idx])) # + torch.mean(torch.abs(self.app_plane[idx])) + torch.mean(torch.abs(self.density_plane[idx]))
        total = total + torch.mean(torch.abs(self.density_grid[0]))
        return total
    ...
    def compute_densityfeature(self, xyz_sampled):
        ...
        grid_coef_point = F.grid_sample(self.density_grid[0], xyz_sampled.view(1,-1,1,1,3),
                                                align_corners=True).view(-1, *xyz_sampled.shape[:1])
        # sigma_feature = torch.zeros((xyz_sampled.shape[0],), device=xyz_sampled.device)
    ...
    @torch.no_grad()
    def up_sampling_VM(self, plane_coef, grid_coef, res_target):
        ...
        for i in range(len(self.vecMode)):
            ...
            plane_coef[i] = torch.nn.Parameter(
                F.interpolate(plane_coef[i].data, size=(res_target[mat_id_1], res_target[mat_id_0]), mode='bilinear',
                              align_corners=True))
        grid_coef[0] = torch.nn.Parameter(
            F.interpolate(grid_coef[0], size=((res_target[0]+3)//4, (res_target[1]+3)//4, (res_target[2]+3)//4), mode='trilinear', align_corners=True))

        return plane_coef, grid_coef

    @torch.no_grad()
    def upsample_volume_grid(self, res_target):
        self.app_plane, self.app_grid = self.up_sampling_VM(self.app_plane, self.app_grid, res_target)
        self.density_plane, self.density_grid = self.up_sampling_VM(self.density_plane, self.density_grid, res_target)
    ...
    def shrink(self, new_aabb):
        ...
        g_t_l, g_b_r = torch.div((xyz_min - self.aabb[0]), self.units*4), torch.div((xyz_max - self.aabb[0]), self.units*4)
        g_t_l, g_b_r = torch.round(torch.round(g_t_l)).long(), torch.round(g_b_r).long() + 1
        g_b_r = torch.stack([g_b_r, torch.div(self.gridSize+3, 4, rounding_mode='trunc')]).amin(0)
        self.density_grid[0] = torch.nn.Parameter(
            self.density_grid[0].data[...,g_t_l[0]:g_b_r[0],g_t_l[1]:g_b_r[1],g_t_l[2]:g_b_r[2]]
        )
        self.app_grid[0] = torch.nn.Parameter(
            self.app_grid[0].data[...,g_t_l[0]:g_b_r[0],g_t_l[1]:g_b_r[1],g_t_l[2]:g_b_r[2]]
        )
